...
同时标注你对本次分类的把握程度：
- `high`：问题清晰，明确属于某一类。
- `low`：问题模糊、信息不足，或在 additional-query 与 graphrag-query 之间难以确定（例如"灯泡坏了怎么办"既像故障排查又像信息不足）。
"""
...
# 图片分析提示（给视觉模型，让它详细描述图片内容）
IMAGE_ANALYSIS_SYSTEM_PROMPT = "你是一个专业的图像分析助手。请详细分析图片中的内容，特别关注产品细节、品牌、型号等信息。"
...
# 文件问答提示（只根据检索到的文件片段回答，不得编造）
FILE_QUERY_SYSTEM_PROMPT = """你是一个电商客服助手。请只根据下面从用户上传文件中检索到的内容回答问题。
如果检索内容里没有相关信息，就如实告诉用户"文件里没有相关信息"，不要编造。
